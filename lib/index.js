...
// [fix 2026-08-20] sops/ 保留名：非 SOP 内容文件（安装/文档文件）不得计入 L3 条目。
const SOP_RESERVED_NAMES = new Set(["readme", "license", "index"]);
// [fix 2026-08-20] 新条目 recency 保护：写入后 RECENCY_WINDOW_MS 内压缩评分 +RECENCY_BONUS，
// 避免"写完即隐身"（0 热度新条目被压缩立刻裁出 L1）。对齐 mem0/MemGPT 的 recency 加权思路。
...
/** sops/ 的文件名列表（去 .md，过滤保留名如 README/LICENSE/index，防非内容文件混入 L3）。 */
...
function setEntryMeta(root, kind, key, patch) {
	...
	store[key] = {
		...prev,
		...patch,
		// [fix 2026-08-20] 补齐 createdAt：首建记录，更新保留原创建时间（此前只写 updatedAt，recency 无据可查）
...
/**
 * 排序/裁剪用热度：显式访问热度优先；近 RECENCY_WINDOW_MS 内创建的新条目
 * 在无访问热度时获得 RECENCY_BONUS，防止"写完即隐身"。
 * meta 由调用方一次性 readMeta 传入，避免逐条重复读文件。
 */
function entryHeat(access, meta, kind, key) {
	const heat = access[`${kind}:${key}`] || 0;
	if (heat > 0) return heat;
	const entry = (kind === "fact" ? meta.facts : meta.sops)[key];
...
function writeMemory(root, { topic, entryType, content, evidence, sourceSession, sourceSeqs, namespace }) {
	...
	// [fix 2026-08-20] 写入即热：显式 bump 一次，叠加 recency 保护，杜绝写完即被裁出 L1
	const writeKey = entryType === "fact" ? safeTopic : slugify(safeTopic);
	bumpAccess(root, `${entryType === "fact" ? "fact" : "sop"}:${writeKey}`);
