...
function detectNamespace() {
	try {
		...
		const lower = cwd.replace(/\\/g, "/").toLowerCase();
		// 本地约定：股票项目（中文路径或英文别名）统一使用 stock 命名空间
		if (lower.includes("股票分析系统") || lower.includes("stock analysis system") || lower.includes("stock-analysis")) {
			return "stock";
		}
...
/** 压缩 L1 索引：清理多余空行；条目不多时全保留，只有条目极多时才按访问热度裁剪（文件不删除）。 */
function compressIndexEntries(root, maxLines) {
	...
	// 本地定制：先清理多余空行（连续空行压到 1 个，尾部只留 1 个换行）
	head = head.replace(/\n{3,}/g, "\n\n").replace(/\n+$/, "\n");
	tail = tail.replace(/\n{3,}/g, "\n\n").replace(/^(\n)+/, "\n").replace(/\s+$/, "\n");
	// 自动段只占 L2/L3 两行，行数超限主要靠清空行解决；只有条目极多时才按热度裁剪
	const totalActive = facts.length + sops.length;
	const maxKept = Math.max(20, maxLines * 2);
	let keptFacts = facts;
	let keptSops = sops;
	if (totalActive > maxKept) {
		const factBudget = Math.min(facts.length, Math.ceil(maxKept / 2));
		const sopBudget = Math.min(sops.length, Math.floor(maxKept / 2));
		keptFacts = facts.slice(0, factBudget);
		keptSops = sops.slice(0, sopBudget);
	}
...
function apply(ctx, config = {}) {
	...
	if (sysPrompt) {
		disposers.push(sysPrompt.context({
			...
			order: 10,
			text: () => {
				// 本地定制：通用记忆（default）与当前项目记忆一起注入，
				// 保证两个项目都能看到共享索引，同时保留项目专属索引。
				const sharedRoot = nsRoot(cfg.memoryDir, "default");
				ensureNamespaceLayout(sharedRoot);
				const sharedIdx = readIndex(sharedRoot).trim();
				const ns = resolveNamespace(cfg);
				const root = nsRoot(cfg.memoryDir, ns);
				ensureNamespaceLayout(root);
				const projectIdx = readIndex(root).trim();
				const parts = [];
				if (sharedIdx) parts.push(`[通用记忆]\n${sharedIdx}`);
				if (projectIdx && ns !== "default") parts.push(`[项目记忆 ${ns}]\n${projectIdx}`);
				return parts.join("\n\n") || "";
			}
		}));
	}
