...
## 分层
- L1 index.txt：AUTO 段由插件全量重建（每层一行、" | " 打包所有活跃条目名），预算是字符数（l1MaxChars）而非行数——被隐藏的条目等于永久隐身，因此存在性不裁剪。超预算时注入视图按分类入口折叠条目并显式标注压缩，规则段完整保留；完整列表始终在 index.txt，可用 memory_list / memory_search 取回。要真正降低总量，仍靠合并/归档条目或精简 [RULES]。
